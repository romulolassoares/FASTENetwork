...
createAnchorPeerUpdate() {
  ...
  fetchChannelConfig $ORG_NAME $CHANNEL_NAME ${CORE_PEER_LOCALMSPID}config.json ${ORG_NAME} ${org_NAME} ${PEER_NUMBER} ${PORT_PEER}

  infoln "Generating anchor peer update transaction for ${ORG_NAME} on channel $CHANNEL_NAME"

  HOST="${PEER_NUMBER}.${org_NAME}.example.com"
  PORT=${PORT_PEER}
...
PEER_NUMBER=$4
PORT_PEER=$5
setGlobalsCLI ${ORG_NAME} ${org_NAME} ${PEER_NUMBER} ${PORT_PEER}
